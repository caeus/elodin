...
import io.github.caeus.felurian.eval.Ctx.FunCtx
...
object ApplyTo {
  ...
  private def continue(
    ...
    remaining.headOption
      .map { arg0 =>
        result match {
          case applicable: Value.Applicable =>
            applicable(arg0, remaining.tail)
          case s => EvalException.applyError(s)
        }
      }
  ...
  def foreign(
      name: String,
      arity: Int,
      reduce: Seq[Value] => ZIO[Resolver, EvalException, Value]
  )(
      arg0: Value,
      args: Seq[Value]
  ): ZIO[Resolver, EvalException, Value] = {
