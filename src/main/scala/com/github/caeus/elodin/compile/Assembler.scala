...
import com.github.caeus.elodin.archive
import com.github.caeus.elodin.archive.Book.CBook
import com.github.caeus.elodin.archive.RefResolution.{FunParam, LetBinding, ModuleMember}
import com.github.caeus.elodin.archive._
import com.github.caeus.elodin.compile.Lexcope._
import com.github.caeus.elodin.runtime.Value
    ...
    def toShift(scope: Lexcope[Node], bundle: NodeBundle): Task[Shift] = {
      scope match {
        ...
        case WhenFloat(lexcope) =>
          Task.succeed(Shift.Of(Value.Atom(lexcope.node.value)))
    ...
    ZIO
      .collectAll(bundles.map { bundle =>
        toShift(bundle.scope, bundle).map { shift =>
          bundle.scope.path -> archive.Shifter(arity = bundle.header.length, shift)
        }
      })
      .map(_.toIndexedSeq)
  }
